...
"function sendRelativeSlew(){const input=$('relative-slew-angle'),angle=Number(input.value);if(!Number.isFinite(angle)||angle<-180||angle>180){$('status').textContent='Relative angle must be between -180 and +180 degrees';input.focus();return}sendCommand('SLEW_REL,'+angle)}$('wheel-stop').onclick=()=>sendCommand('SET_VALUE,0');$('slew-abort').onclick=()=>sendCommand('SLEW_ABORT');$('angle-reset').onclick=()=>sendCommand('ANGLE_RESET');$('relative-slew').onclick=sendRelativeSlew;$('relative-slew-angle').onkeydown=e=>{if(e.key==='Enter'){e.preventDefault();sendRelativeSlew()}};"
