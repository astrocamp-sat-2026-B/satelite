...
"<section class=\"section flight-controls\"><div class=\"head\"><h2>Flight controls</h2><span class=\"meta\">Commands are sent directly to Pico</span></div><div class=\"flight-actions\"><button id=\"wheel-stop\" class=\"danger\" type=\"button\">Wheel stop</button><button id=\"slew-abort\" class=\"danger\" type=\"button\">Abort slew</button><button id=\"angle-reset\" type=\"button\">Angle reset</button><label>Relative angle <input id=\"relative-slew-angle\" type=\"number\" min=\"-180\" max=\"180\" step=\"1\" value=\"1\"> deg</label><button id=\"relative-slew-capture\" class=\"primary\" type=\"button\">Slew &amp; capture</button></div><span class=\"meta\">Relative slew is limited to -180 to +180 degrees. Angle reset is rejected during an active slew.</span></section>"
...
"function sendRelativeSlewCapture(){const input=$('relative-slew-angle'),angle=Number(input.value);if(!Number.isFinite(angle)||angle<-180||angle>180){$('status').textContent='Relative angle must be between -180 and +180 degrees';input.focus();return}sendCommand('SLEW_REL_CAPTURE,'+angle)}$('wheel-stop').onclick=()=>sendCommand('SET_VALUE,0');$('slew-abort').onclick=()=>sendCommand('SLEW_ABORT');$('angle-reset').onclick=()=>sendCommand('ANGLE_RESET');$('relative-slew-capture').onclick=sendRelativeSlewCapture;$('relative-slew-angle').onkeydown=e=>{if(e.key==='Enter'){e.preventDefault();sendRelativeSlewCapture()}};"
