...
"<section class=\"charts graph-charts\"><article class=\"section\"><div class=\"head\"><h2>Temperature</h2><span class=\"meta\">auto scale</span></div><svg id=\"temp-chart\" class=\"chart\" viewBox=\"0 0 720 250\"></svg></article><article class=\"section\"><div class=\"head\"><h2>Gyro Z</h2><span class=\"meta\">auto scale</span></div><svg id=\"gyro-chart\" class=\"chart\" viewBox=\"0 0 720 250\"></svg></article><article class=\"section\"><div class=\"head\"><h2>Photodiode ADC</h2><span class=\"meta\">auto scale</span></div><svg id=\"pd-chart\" class=\"chart\" viewBox=\"0 0 720 250\"></svg></article><article class=\"section\"><div class=\"head\"><h2>Photoreflector</h2><span class=\"meta\">auto scale</span></div><svg id=\"photoreflector-chart\" class=\"chart\" viewBox=\"0 0 720 250\"></svg></article></section></aside>"
...
"const fields=[['Uptime',v=>v.uptime_s,' s',0],['Temperature',v=>v.temp_c,' C',2],['Gyro Z',v=>v.gyro_z_dps,' dps',2],['Gyro angle',v=>v.gyro_z_angle_deg,' deg',2],['Command',v=>v.command_value,'',0],['PD0',v=>v.photodiode_adc?.[0],'',0],['PD1',v=>v.photodiode_adc?.[1],'',0],['PD2',v=>v.photodiode_adc?.[2],'',0],['PD3',v=>v.photodiode_adc?.[3],'',0],['Photoreflector',v=>v.photoreflector_adc,'',0],['Wi-Fi signal',v=>v.wifi_signal_quality,' %',0]];"
...
"async function sendSequenceCommand(command){const response=await fetch('/api/command',{method:'POST',headers:{'Content-Type':'text/plain'},body:command}),result=await response.json();if(!response.ok||!result.ok)throw Error('Pico is not connected');loaded=false}"
